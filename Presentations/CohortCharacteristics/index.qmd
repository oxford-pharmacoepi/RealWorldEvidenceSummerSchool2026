...
Lets create a reference to the *delphi* dataset, remember that the results shown in this presentation are created using a different dataset (*GiBleed*).
...
It return the cohort counts in a `summarised_result` object format.
...
Note that the plot functions are ggplot2 objects that you can later edit (if you are familiar with the ggplot2 interface), For example here we changed the legend and the axis titles:
...
It return the cohort attrition in a `summarised_result` object format.
