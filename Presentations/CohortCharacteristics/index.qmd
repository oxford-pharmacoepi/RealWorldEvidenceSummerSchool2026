...
- Add the medications that they took the last 180 days.
- Add the conditions that were recorded in the last 2 years.
- Customise with time to / time from another event.
...
result <- summariseCharacteristics(
  cohort = cdm$new_sinusitis,
  cohortIntersectFlag = list(
      "Conditions prior 2 years" = list(
        targetCohortTable = "conditions", 
        window = c(-730, 0)
      ),
      "Medications prior 180 days" = list(
        targetCohortTable = "medications", 
        window = c(-180, 0)
      )
    )
...
Can you create a bar plot the percentage of sex in the `sinusitis` table?
...
Can you get the large-scale characteristics from the `drug_exposure` table for the `new_sinusitis` cohort table using an anytime-prior time window? Visualise the result showing the top 5 most frequent codes.
